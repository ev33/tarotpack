	...
	const faqList = [
		{
			...
			answer:
				"타로팩은 매일 카카오톡 메세지 링크를 통해 타로팩을 뽑아 운세 풀이를 받아보는 서비스예요. 타로팩을 통해 하루의 흐름을 미리 점쳐보고 긍정적인 에너지를 얻는 습관을 만들어보세요."
		},
		{
		...
		},
		{
			...
			answer:
				"환불 요청 시점이 결제일로부터 7일 이내라면 이용 기간을 제외한 남은 기간을 일할 계산하여 부분 환불이 가능해요. 하지만 결제 후 7일이 지났다면 환불이 어려워요.\n\n환불을 원하신다면 타로팩 고객센터 이메일(cs@tarotpack.com)로 구매 시 입력하셨던 성함과 전화번호를 보내주세요."
		}
	];
